    ...
    <View style={{ flex: 1, backgroundColor: colours.bg }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <Card borderColor={tripSharing ? colours.accent : colours.border}>
          ...
          {!tripSharing ? (
            <TouchableOpacity style={{ margin: 16, marginTop: 0, backgroundColor: colours.accent, borderRadius: 12, paddingVertical: 14, alignItems: 'center' }} onPress={startTripSharing}>
              <Text style={{ color: 'white', fontWeight: '700', fontSize: fonts.lg }}>🛡️ {t('Start Trip Sharing', 'Démarrer le partage')}</Text>
            </TouchableOpacity>
          ) : (
            <View style={{ padding: 16, paddingTop: 0, gap: 10 }}>
              <TouchableOpacity style={{ backgroundColor: colours.accent + '18', borderWidth: 1, borderColor: colours.accent + '40', borderRadius: 12, paddingVertical: 12, alignItems: 'center' }} onPress={shareLocation}>
              ...
              <TouchableOpacity style={{ borderWidth: 1, borderColor: colours.border, borderRadius: 12, paddingVertical: 12, alignItems: 'center' }} onPress={stopTripSharing}>
        ...
        <Card>
          {[
            { icon: '🚨', title: t('Call 911', 'Appeler le 911'), desc: t('Police, ambulance, fire', 'Police, ambulance, pompiers'), tel: '911' },
            { icon: '🚌', title: t('OC Transpo Info', 'Info OC Transpo'), desc: '613-560-1000', tel: '6135601000' },
            { icon: '🏥', title: t('Ottawa Hospital', "Hôpital d'Ottawa"), desc: '613-828-1275', tel: '6138281275' },
          ].map((item, i) => (
            <View key={i}>
              {i > 0 && <Divider />}
              <TouchableOpacity
                ...
                <Text style={{ fontSize: fonts.lg, color: colours.muted }}>→</Text>
              </TouchableOpacity>
            </View>
          ))}
        </Card>
        ...
        <Card>
          {[
            { icon: '💡', tip: t('Wait under lights — avoid dark shelters late at night', 'Attendez sous les lumières — évitez les abris sombres') },
            { icon: '👥', tip: t('Board the first or second car — closer to the operator', 'Montez dans le premier ou deuxième wagon') },
            { icon: '📱', tip: t('Keep your phone charged and location services on', 'Gardez votre téléphone chargé et la localisation activée') },
            { icon: '🚉', tip: t('Hurdman and Bayview have 24h security cameras', 'Hurdman et Bayview ont des caméras de sécurité 24h') },
            { icon: '🔔', tip: t('Tell someone your route before boarding late night', "Dites à quelqu'un votre trajet avant de partir tard") },
          ].map((item, i) => (
            <View key={i}>
              {i > 0 && <Divider />}
          ...
          <View style={{ flexDirection: 'row', gap: 8, paddingHorizontal: 16, paddingBottom: 14 }}>
            {(['dark', 'light', 'system'] as const).map(th => (
              <TouchableOpacity
                ...
                onPress={() => setTheme(th)}>
                <Text style={{ fontSize: 20 }}>{themeIcons[th]}</Text>
